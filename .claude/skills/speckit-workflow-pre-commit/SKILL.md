...
description: "Run closure-auditor at the pre-commit checkpoint for final audit-trail verification before git commit"
argument-hint: "Optional: feature directory path (defaults to active feature)"
compatibility: "Requires spec-kit project structure with .specify/ directory and the closure-auditor sub-agent"
metadata:
...
```
@closure-auditor I'm at the pre-commit checkpoint for the current feature. Please verify all closure evidence is valid and the commit is safe to proceed.
```
